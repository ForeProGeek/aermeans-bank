...
export default function AuthScreen({ onAuthSuccess }) {
  ...
  const inputClass =
    "w-full bg-navy-700 border border-navy-600 rounded-xl px-4 py-3 pl-11 text-white placeholder-gray-400 focus:outline-none focus:border-gold transition-colors";

  return (
    <div className="h-full w-full bg-navy-900 flex flex-col px-6 py-8 overflow-y-auto scrollbar-hide">
      {/* Logo */}
      <div className="flex flex-col items-center mb-8">
        <div className="w-16 h-16 bg-gold rounded-2xl flex items-center justify-center mb-3">
          <span className="text-navy-900 font-bold text-2xl">æ</span>
        </div>
        <h1 className="text-2xl font-bold text-white">Aermeans</h1>
        <p className="text-gray-400 text-sm mt-1">Experience Seamless Transactions</p>
      </div>
      ...
      {mode === "login" && (
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-gold rounded-xl text-navy-900 font-bold text-base hover:bg-gold-light transition-colors disabled:opacity-50"
          >
            {loading ? "Logging in..." : "Log In"}
          </button>
          <p className="text-center text-gray-400 text-xs">
            Demo: username <span className="text-gold">joshua</span> / password <span className="text-gold">123456</span>
          </p>
        </form>
      )}
