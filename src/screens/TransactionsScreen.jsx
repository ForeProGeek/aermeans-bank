  ...
  return (
    <div className="min-h-full pb-24">
      ...
      <div className="px-4 py-4 border-b border-navy-600">
        <h2 className="font-family-script text-white text-2xl text-center">Transactions</h2>
      </div>

      {/* Filter */}
      <div className="flex items-center justify-end gap-4 px-4 py-3">
        <button className="flex items-center gap-2 text-gold font-family-script">
      ...
      <div className="px-4 space-y-3">
        {transactions.map((tx) => (
          <div
            key={tx.id}
            className="flex items-center gap-3 bg-navy-700/50 rounded-xl p-3"
          >
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                tx.type === "debit" ? "bg-blue-500/20" : "bg-green-500/20"
              }`}
            >
              {tx.type === "debit" ? (
            ...
            <div className="flex-1 min-w-0">
              <p className="font-family-script text-white text-sm truncate">{tx.title}</p>
              <p className="font-family-script text-gray-400 text-xs">{tx.date}</p>
            </div>
            <div className="text-right">
              <p
                className={`font-family-script text-sm font-semibold ${
                  tx.type === "debit" ? "text-red-400" : "text-green-400"
                }`}
              >
                {tx.amount}
              </p>
              <p className="font-family-script text-gray-400 text-xs">{tx.balance}</p>
            </div>
          </div>
        ))}
      </div>
